    ...
    @else
        @foreach ($children as $child)
            ...
            <div class="w-70 h-30 flex flex-col bg-white rounded-lg shadow-md py-2 px-2">
                ...
                <div>
                    <img class="mt-5 rounded-md image" src="{{ $child->profile_picture }}" alt="{{ $child->first_name }}">
                </div>
                <h3 class="  w-full  text-center name">{{ $child->first_name }}
...
<script>
    $(document).ready(function() {
        @foreach ($children as $child)
            var hobby = {!! json_encode($child->hobby) !!};
            var dob = {!! json_encode($child->date_of_birth) !!};
            var name1 = {!!  json_encode($child->first_name) !!}
            var name2 = {!!  json_encode($child->second_name) !!}
            //jpin both names into name
            var name = name1 + ' ' + name2;
           
             console.log("Name: " + name);
            var storys = {!! json_encode($child->story) !!}; // Properly encode the story here
